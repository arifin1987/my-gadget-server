...
async function run() {
  try {
    ...
    const laptopCollection = client.db("my-gadget").collection("laptop");
    ...
    app.get("/laptop/:laptopId", async (req, res) => {
...
app.get("/", (req, res) => {
  res.send("my gadget server is running");
});

app.listen(port, () => {
  console.log(`my gadget server app listening on port ${port}`);
});
